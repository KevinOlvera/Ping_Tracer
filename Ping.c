...
int Gateway_Address(unsigned char *gatewayIParray)
{
...
void IP_Header(unsigned char *trama, unsigned int protocol, unsigned char *sourceIPAddr, unsigned char *targetIPAddr, int count, unsigned int ttl)
{
    struct timeval time;
...
void ICMP_Header(unsigned char *trama, unsigned short seqNumber, int ID)
{
    unsigned char identifier[2];
    unsigned char checkSum[2];
    unsigned char data[32] = {"Kevin Jesus Olvera Olvera"}; //payload
    unsigned short ckSum;
...
void rcvARP(unsigned char *trama_rcv, unsigned char *sourceHwAddr, unsigned char *targetIPAddr, int trama_len, int *ifindex, int ds, unsigned char *targetHwAddr)
{
    ...
    while (mtime < 300)
    {
        ...
        {
            memcpy(targetHwAddr, trama_rcv + 22, 6);
            bandera = 1;
            close(ds);
        }
...
int ICMP(unsigned char *trama_icmp, int trama_len, int protocolNumber, unsigned char *sourceIPAddr, unsigned char *targetIPAddr, int sqNumber, int pID, int ds, int index, unsigned int ttl, int printsetup)
{
    ...
    IP_Header(trama_icmp, protocolNumber, sourceIPAddr, targetIPAddr, sqNumber, ttl);
...
int filterICMPreply(unsigned char *trama, int pID, unsigned short seqNumber, int tam_rcv_from, int printsetup, unsigned int ttl, struct timeval *tval_now)
{
    ...
    else if (!memcmp(trama + 12, ETHTYPE_ICMP, 2) && !memcmp(trama + 34, "\x0b\x00", 2) && !memcmp(trama + 66, identifier, 2) && !memcmp(trama + 69, sqnum, 1))
    { //Respuesta desde hop
        gettimeofday(&tval_after, NULL);
        timersub(&tval_after, tval_now, &tval_result);
        float timeres = (long int)tval_result.tv_usec / 1000.0;
